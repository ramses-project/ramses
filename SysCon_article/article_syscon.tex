...
An evolutionary algorithm (EA) is characterized by three features :
\begin{itemize}
\item a set of solution candidates is maintained,
\item a mating selection process is performed on this set, and
\item several solutions may be combined in terms of recombination to generate new solutions.
\end{itemize}

The solution candidates are called \textbf{genomes} (or individuals) and the set of solution candidates is called the \textbf{population}. Each genome represents a possible solution, i.e., a decision vector, to the problem at hand; however, a genome is not a decision vector but rather encodes it based on an appropriate representation.
...
Then, the variation operators are applied to the mating pool. Usually these operators are called \textbf{genetic operators}. With EAs, there are two operators namely the crossover (or recombination) and the mutation.
...
\subsection{Fitness evaluation of each genome}
...
Figure~\ref{binary} illustrates this operation. On this figure, we suppose we have four genomes G1,G2,G3 and G4 respectively composed of TRA11,TRA21,TRA31 and TRA41. We apply two tournaments between paired genomes to produce the needed genomes. In the first round we have two tournaments between two different pairs of genomes (G1 with G2 and G3 with G4). In the second round we apply another tournament on the victorious genomes in the previous round (G2 and G4) . 

The final genomes of the competition will always be the fittest genomes in the population. The more successful the genome is in competition, the more often that genome is expected to appear in the mating pool (In Figure~\ref{binary} the fittest genome is G4).
