...
public class AadlToOSEKCUnparser implements AadlTargetUnparser {
	...
	public TargetProperties process(SystemImplementation si, File generatedFilePath) throws GenerationException {
	  ...
	  File configurationPath = FileUtils.getContainingDirectory(si.eResource());
	...
	public void process(SubprogramType elt, File generatedFilePath) {
		...
		// Add C source location on oil
		this.processSourceLocation(elt, os);
		
	}

	/**
	 * Set all generated c files.
	 */
	public void process(List<File> list) {
		oil.getCpu().getOs().addAllAppSrcs(list);
	}
	...
	private void genOsConfig(ProcessSubcomponent processSubcomponent) {
		...
		os.addAppSrc(new File(_generatedCodeDirectory, "main.c"));
		os.addAppSrc(new File(_generatedCodeDirectory, "activity.c"));
		os.addAppSrc(new File(_generatedCodeDirectory, "deployment.c"));
		os.addAppSrc(new File(_generatedCodeDirectory, "gtypes.c"));
		os.addAppSrc(new File(_generatedCodeDirectory, "subprograms.c"));
	...
	private void processSourceLocation(NamedElement elt, Os os)
	{
	  List<String> srcs_SL = new ArrayList<String>();
	  List<String> srcs_ST = new ArrayList<String>();
	  try {
	    srcs_SL = PropertyUtils.getStringListValue(elt, "Source_Location");
	  } catch (Exception exception) {

	  }
	  try {
	    srcs_ST = PropertyUtils.getStringListValue(elt, "Source_Text");
	  } catch (Exception exception) {
	...
	private void genTask(ProcessSubcomponent ps, ThreadSubcomponent thread, String counterName) {
		...
		processSourceLocation(thread, os);
    ...
    try
    {
      ...
      FileUtils.saveFile(generatedFilePath, process.getName() + "_specification.oil",
